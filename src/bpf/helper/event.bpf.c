...
} AMEBA_MAP_NAME_CONTROL SEC(".maps");
static void *control_map = &AMEBA_MAP_NAME_CONTROL;
...
struct
{
    ...
    __type(key, int);
    __type(value, struct control);
} AMEBA_TASK_MAP_NAME(storage_task_control) SEC(".maps");
static void *storage_task_control = &AMEBA_TASK_MAP_NAME(storage_task_control);
...
int event_init_context(struct event_context *e_ctx, record_type_t r_type)
{
    ...
    int key = 0;
    struct control *control_ptr = bpf_map_lookup_elem(control_map, &key);
    if (control_ptr)
    {
        struct task_struct *current_task = (struct task_struct *)bpf_get_current_task_btf();
        void *task_control_ptr = bpf_task_storage_get(
            storage_task_control, current_task, control_ptr, BPF_LOCAL_STORAGE_GET_F_CREATE
        );
        if (task_control_ptr)
        {
            __builtin_memcpy(task_control_ptr, control_ptr, sizeof(struct control));
        } else
        {
            LOG_ERROR("Failed to allocation per task storage for control");
        }
...
static int is_int_in_control_id_list(int needle, int *haystack, const int haystack_len)
{
...
static int is_task_auditable(struct task_struct *current, struct control *runtime_control)
{
...
int event_is_netio_set_to_ignore(struct control *runtime_control)
{
    if (!runtime_control)
        return 0;
...
static int is_audit_log_exit_syscall_auditable(struct task_struct *current_task, struct control *runtime_control)
{
    if (!current_task)
        return 0;
...
int event_is_auditable(struct event_context *e_ctx)
{
    ...
    struct control *ci = bpf_task_storage_get(
        storage_task_control, current_task, 0, 0
    );
    if (!ci)
        return 0;

    control_trace_mode_t global_mode = ci->global_mode;
