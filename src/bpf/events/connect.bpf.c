...
// externs
extern int recordhelper_zero_out_record_connect(
    struct record_connect *r_connect
);
extern int recordhelper_init_record_connect(
    struct record_connect *r_connect,
...
)
{
    ...
    recordhelper_init_record_connect(
        map_val, 
        pid,
        fd,
        ret
    ...
    bpf_probe_read_user(&(remote_sa->addr[0]), remote_sa->addrlen, sockaddr);

    LOG_WARN("[magic_test] AMEBA_MAGIC=%d", AMEBA_MAGIC);
