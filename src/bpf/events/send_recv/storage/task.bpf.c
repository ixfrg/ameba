...
} AMEBA_TASK_MAP_NAME(storage_send_recv) SEC(".maps");
static void *storage_send_recv = &AMEBA_TASK_MAP_NAME(storage_send_recv);
...
int send_recv_storage_insert(struct record_send_recv *map_val)
{
    ...
    void *result = bpf_task_storage_get(storage_send_recv, current_task, map_val, BPF_LOCAL_STORAGE_GET_F_CREATE);
    return result != NULL;
}
...
{
    struct task_struct *current_task = (struct task_struct *)bpf_get_current_task_btf();
    return bpf_task_storage_delete(storage_send_recv, current_task);
}
...
{
    struct task_struct *current_task = (struct task_struct *)bpf_get_current_task_btf();
    struct record_send_recv *result = bpf_task_storage_get(storage_send_recv, current_task, 0, 0);
    if (!result)
        return 0;
...
{
    struct task_struct *current_task = (struct task_struct *)bpf_get_current_task_btf();
    struct record_send_recv *result = bpf_task_storage_get(storage_send_recv, current_task, NULL, 0);
    if (!result)
        return 0;
