...
unsigned long increment_event_id()
{
...
static void bpf_map_update_unconditionally(void *map, void *key, void *val)
{
    long updated = bpf_map_update_elem(map, key, val, BPF_ANY);
    if (updated != 0)
    {
        log_warn("Failed to update map 'process_record_map'");
    }
}


static void bpf_map_delete_unconditionally(void *map, void *key)
{
    long map_delete = bpf_map_delete_elem(map, key);
    if (map_delete != 0)
    {
        log_warn("Failed to delete map 'process_record_map'");
    }
}
...
)
{
    ...
    bpf_map_update_unconditionally(&process_record_map, &map_key, (void *)&map_val);
...
)
{
    ...
    // bpf_ringbuf_output(&ringbuf, map_val, sizeof(struct record_connect), 0);

    bpf_map_delete_unconditionally(&process_record_map, &map_key);
