...
static struct read_input_arg global_read_input_arg;
static struct read_input global_read_input_initial_value;
...
struct argp global_read_input_argp = {
    .options = options,
    .parser = parse_opt,
    .args_doc = "",
...
static struct read_input_arg *get_global_read_input_arg()
{
    return &global_read_input_arg;
}

static struct read_input *get_global_read_input_initial_value()
{
    return &global_read_input_initial_value;
}

static void init_read_input(struct read_input_arg *input)
{
    if (!input)
        return;
...
static error_t parse_opt(int key, char *arg, struct argp_state *state)
{
    struct read_input_arg *input = get_global_read_input_arg();
    ...
    case ARGP_KEY_END:
        validate_read_input(input, state);
        break;
...
static void user_args_read_copy(struct read_input_arg *dst)
{
    if (!dst)
        return;
    memcpy(dst, get_global_read_input_arg(), sizeof(struct read_input_arg));
}
...
void user_args_read_parse(struct read_input_arg *dst, struct read_input *initial_value, int argc, char **argv)
{
    ...
    if (!initial_value)
        memset(&global_read_input_initial_value, 0, sizeof(struct read_input));
    else
        memcpy(&global_read_input_initial_value, initial_value, sizeof(struct read_input));

    int argp_flags = 0;
    // ARGP_NO_EXIT & ARGP_NO_HELP because self-managed
    argp_flags = ARGP_NO_EXIT | ARGP_NO_HELP;
    argp_parse(&global_read_input_argp, argc, argv, argp_flags, 0, 0);

    user_args_read_copy(dst);
}
