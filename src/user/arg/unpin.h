...
/*
    ...
    A module to help parse unpin input from user/config arguments.
...
struct arg_unpin_with_parse_state
{
    struct arg_parse_state parse_state;
    struct arg_unpin arg;
};

/*
    Parse user arguments (i.e. int main(int argc, char **argv)), and populate dst.

    'initial_value' is the value to which (struct arg_unpin_with_parse_state)->(struct arg_unpin)
    ...
    (struct arg_unpin_with_parse_state)->(struct arg_parse_state).
*/
int arg_unpin_parse(
