...
/*
    Globals.
*/
static struct arg_pin_with_parse_state global_arg_with_parse_state;
static struct arg_pin global_arg_initial_value;
...
static struct arg_pin_with_parse_state *get_global_arg_with_parse_state()
{
    return &global_arg_with_parse_state;
}

static struct arg_pin *get_global_arg_initial_value()
{
    return &global_arg_initial_value;
}

static void set_global_initial_value(struct arg_pin *src)
{
    if (!src)
        memset(get_global_arg_initial_value(), 0, sizeof(struct arg_pin));
    else
        memcpy(get_global_arg_initial_value(), src, sizeof(struct arg_pin));
}

static void copy_global_parsed_value_with_state(struct arg_pin_with_parse_state *dst)
{
    memcpy(dst, get_global_arg_with_parse_state(), sizeof(struct arg_pin_with_parse_state));
}

static void initialize_arg_with_parse_state(struct arg_pin_with_parse_state *src)
{
    if (!src)
        return;
    memcpy(&src->arg, get_global_arg_initial_value(), sizeof(struct arg_pin));
    arg_parse_state_init(&(src->parse_state));
    arg_common_init(&src->common);
}

static void validate_arg(struct arg_pin_with_parse_state *src, struct argp_state *state)
{
    // Nothing
}

static void handle_argp_key_end(struct arg_pin_with_parse_state *src, struct argp_state *state)
{
...
static error_t parse_opt(int key, char *arg, struct argp_state *state)
{
    struct arg_pin_with_parse_state *src = get_global_arg_with_parse_state();
...
)
{
    if (!dst)
    {
        fprintf(stderr, "Failed arg_pin_parse. NULL argument(s)");
        return -1;
    }
