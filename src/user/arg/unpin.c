...
/*
    Globals.
*/
static struct arg_unpin_with_parse_state global_arg_with_parse_state;
static struct arg_unpin global_arg_initial_value;
...
static struct arg_unpin_with_parse_state *get_global_arg_with_parse_state()
{
    return &global_arg_with_parse_state;
}

static struct arg_unpin *get_global_arg_initial_value()
{
    return &global_arg_initial_value;
}

static void set_global_initial_value(struct arg_unpin *src)
{
    if (!src)
        memset(get_global_arg_initial_value(), 0, sizeof(struct arg_unpin));
    else
        memcpy(get_global_arg_initial_value(), src, sizeof(struct arg_unpin));
}

static void copy_global_parsed_value_with_state(struct arg_unpin_with_parse_state *dst)
{
    memcpy(dst, get_global_arg_with_parse_state(), sizeof(struct arg_unpin_with_parse_state));
}

static void initialize_arg_with_parse_state(struct arg_unpin_with_parse_state *src)
{
    if (!src)
        return;
...
static error_t parse_opt(int key, char *arg, struct argp_state *state)
{
    struct arg_unpin_with_parse_state *src = get_global_arg_with_parse_state();
...
int arg_unpin_parse(
    struct arg_unpin_with_parse_state *dst,
...
)
{
    if (!dst)
    {
        fprintf(stderr, "Failed arg_unpin_parse. NULL argument(s)");
        return -1;
    }
